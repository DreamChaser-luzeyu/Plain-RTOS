...
uint8_t ScheduleProcess_Flag = 0;
...
//    printf("[DEBUG] Old PCB addr: %08lx, New PCB addr: %08lx\r\n", CurrentProcess_Ptr_GV, NextProcess_Ptr_GV);
    ...
    CurrentProcess_Ptr_GV = NextProcess_Ptr_GV;
    __asm__(
            "LDR    R0,     %0\n\t"             // Load psp to R0 register from variable
            "LDMIA  R0!,    {R4-R11}\n\t"       // Load R4-R11 from the stack of the next thread
            "MSR    PSP,    R0\n\t"             // Set psp to the next stack
            : /* - No output - */
            : "m"(NextProcess_Ptr_GV->processStackPointer));
...
//        printf(" %d ", tick_counter);
        DoContextSwitchInterrupt_Func();
//        ScheduleProcess_Flag = 1;
        tick_counter = 0;
    }
